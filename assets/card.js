...
jQuery(function() {
    ...
    (CardJs.copyAllElementAttributes = function(source, destination) {
        jQuery.each(source[0].attributes, function(idx, attr) {
    ...
    (CardJs.prototype.setupExpiryInput = function() {
        ...
        if (this.EXPIRY_USE_DROPDOWNS) {
            ...
            for (var expiryYearNew = jQuery("<select><option value='any' selected='' hidden=''>YY</option></select>"), currentYear = parseInt(new Date().getFullYear().toString().substring(2, 4)), i = 0; i < CardJs.EXPIRY_NUMBER_OF_YEARS; i++)
    ...
    (CardJs.detachOrCreateElement = function(parentElement, selector, html) {
        ...
        return element[0] ? element.detach() : (element = jQuery(html)), element;
    }),
